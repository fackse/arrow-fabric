...
public class FabricClient {
    ...
    /*public void getTicket(String type, String name){
        GetTicket request = GetTicket.newBuilder().setType("int").setName(name).build();
        Ticket ticket;
        try {
            ticket = blockingStub.getTicket(request);
    ...
    public static void main(String[] args) throws Exception {
        ...
        ManagedChannel dispatcher_channel = ManagedChannelBuilder.forTarget(dispatcher)
                // Channels are secure by default (via SSL/TLS). For the example we disable TLS to avoid
                // needing certificates.
                .usePlaintext()
                .build();
        try {
            FabricClient client = new FabricClient(dispatcher_channel);
            String name = "name1";
